...
**Date:** 2026-04-13
**Branch:** `forest-ui`
**Scope:** Migrate the real app (home, library, readers, reader home) to the approved wireframes in `/admin/design/*`. Add a component-snapshot test harness documented for review.
...
- Split `/h/[householdId]/dashboard` (tabbed Library + Readers) into two independent routes:
  ...
  - `/h/[householdId]/readers` — one panel per reader with `ReaderBookRow`s, "Add reader" CTA
- `/h/[householdId]/dashboard` → **redirect** to `/h/[householdId]/library` (preserve external links, in-app links migrate)
- Rename `/h/[householdId]/kid/[kidId]` → `/h/[householdId]/reader/[readerId]` (route + param)
...
- Strip body: `ReaderActionCard` per reader + trailing `UploadActionCard`.
- Empty state (no readers) now routes to `/readers` (previously `/dashboard`).
...
- Client component `Library` renders:
  - `AppHeader` with `backHref=/h/{householdId}` + `right=ProfileAvatar(currentPath=library)`
  - Compact `UploadCard` row
  - Vertical list of `BookCard variant="parent"` + per-kid progress strip.

### 2.6 Readers page (`/h/[householdId]/readers`) — **new**

- Server component fetches `kids` + `books` (+ per-kid progress) — identical data shape to current `dashboard/page.tsx`.
- Client component `Readers` renders:
  - `AppHeader` with `backHref=/h/{householdId}` + `right=ProfileAvatar(currentPath=readers)`
  - One reader panel per kid (card with avatar, name, "N books · Last active X", list of `ReaderBookRow`, "Edit" cog)
  - Trailing "Add a reader" dashed CTA
- Reuses `AddKidDialog` + `EditKidDialog` as-is.

### 2.7 Reader home (`/h/[householdId]/reader/[readerId]`) — **renamed from kid home**

- `AppHeader` with `backHref` only (no right slot — reader surface, no parent menu).
- Greeting block unchanged.
- Grid of `BookCard variant="reader"` using the harmonized `h-44` hero (see 2.9).

### 2.8 Home strip cards

- `ReaderActionCard` (`components/ReaderActionCard.tsx`) — renamed from `KidCard`. Already rewritten to Strip Companion anatomy per prior work (populated variant, empty variant).
- `UploadActionCard` (`components/UploadActionCard.tsx`) — renamed from `UploadCard` in `HomeCard.tsx`. Already rewritten to Strip Companion anatomy.
- `HomeCard.tsx` is **deleted** — each component gets its own file.

### 2.9 `BookCard` — both variants

**Reader variant** (renamed from "kid"): drop `aspect-[2/3]` cover. Replace with `relative h-44 overflow-hidden` hero that contains:
...
### 2.11 `UploadCard` — **renamed from `BookUpload`**

`client/components/UploadCard.tsx`. Renamed from `BookUpload.tsx`. Same component, new name + file. Used in Library page (compact row).

### 2.12 `KidSelector` — **remove**
...
### 2.13 Absent from scope

- AnimatedOrb rework — separate future plan.
- Visual polish to `EditKidDialog` / `AddKidDialog` — unchanged.
...
- **Scope:** component-level only (`AppHeader`, `ProfileAvatar`, `ReaderActionCard`, `UploadActionCard`, `BookCard`, `ReaderBookRow`). No page-level or route tests. No e2e. No Storybook.
...
- `client/components/ReaderActionCard.tsx` (extracted from `HomeCard.tsx`)
- `client/components/UploadActionCard.tsx` (extracted from `HomeCard.tsx`)
- `client/components/ReaderBookRow.tsx`
- `client/components/UploadCard.tsx` (renamed from `BookUpload.tsx`)
- `client/app/h/[householdId]/library/page.tsx` (server)
- `client/app/h/[householdId]/library/library.tsx` (client)
...
- `client/app/h/[householdId]/readers/readers.tsx` (client)
- `client/app/h/[householdId]/readers/loading.tsx`
- `client/app/h/[householdId]/readers/error.tsx`
- `client/app/h/[householdId]/reader/[readerId]/page.tsx` (server)
- `client/app/h/[householdId]/reader/[readerId]/reader-home.tsx` (client)
- `client/app/h/[householdId]/reader/[readerId]/loading.tsx`
- `client/app/h/[householdId]/reader/[readerId]/error.tsx`
- `client/app/h/[householdId]/reader/[readerId]/call/page.tsx` (moved from kid)
...
- `client/components/__tests__/ReaderActionCard.test.tsx`
- `client/components/__tests__/UploadActionCard.test.tsx`
...
- `client/components/BookCard.tsx` — variant `"kid"` → `"reader"`, hero `h-44`, parent per-kid progress rows.
- `client/app/h/[householdId]/home.tsx` (renamed from `home-page.tsx`) — drop kid circles; drop footer; use `ReaderActionCard` + `UploadActionCard`.
- `client/app/h/[householdId]/page.tsx` — import from `./home` instead of `./home-page`.
...
- `client/components/HomeCard.tsx` (split into `ReaderActionCard.tsx` + `UploadActionCard.tsx`)
- `client/components/BookUpload.tsx` (renamed to `UploadCard.tsx`)
...
- `client/app/h/[householdId]/kid/` (entire directory — moved to `reader/`)
...
- Manual: `/h/$me`, `/h/$me/library`, `/h/$me/readers`, `/h/$me/reader/$id` render correctly against the mockup in `/admin/design/pages`.
